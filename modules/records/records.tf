...
data "aws_route53_zone" "hosted_zone" {
  count = var.create_records && var.zone_id == null  ? 1 : 0
...
resource "aws_route53_record" "record" {
  for_each = { for k, v in local.recordsets : k => v if var.create_records && (var.zone_id != null || var.zone_name != null) }

  zone_id = var.zone_id == null ? data.aws_route53_zone.hosted_zone[0].zone_id : var.zone_id

  name                             = each.value.name != "" ? (lookup(each.value, "full_name_override", false) ? each.value.name : "${each.value.name}.${var.zone_name}") : data.aws_route53_zone.hosted_zone[0].name
  ...
  dynamic "alias" {
    ...
    content {
      ...
      zone_id                = try(each.value.alias.zone_id, data.aws_route53_zone.hosted_zone[0].zone_id)
